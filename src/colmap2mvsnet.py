...
if __name__ == '__main__':
    ...
    parser.add_argument('--modified', action='store_false', default=True)
    ...
    parser.add_argument('--verbose', action='store_true', default=False, help='Print output examples.')
    ...
    depth_intervals = np.zeros(num_images)
    depth_ranges = {}
    ...
    if not args.modified:    
        depth_interval = np.mean(depth_intervals)
        depth_min = np.mean(depth_mins)
        for i in range(num_images):
            depth_ranges[i+1] = (depth_min, depth_interval) # , depth_num, depth_max)
    ...
    try:
        ...
        print(renamed_dir + ' already exists.')
    for i in range(num_images):
